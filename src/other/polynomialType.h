...
#ifndef MAIEV_POLYNOMIALTYPE_H
#define MAIEV_POLYNOMIALTYPE_H

class polynomialType : public arrayListType<double>{
    friend ostream& operator<<(ostream&, const polynomialType&);
    friend istream& operator>>(istream&, const polynomialType&);
public:
...
polynomialType polynomialType::operator-(const polynomialType &right) {
    ...
    if (size == length){
        for(int i = min(length, right.length); i< size; i ++){
            temp.list[i] = list[i];
        }
    }else{
        for(int i = min(length, right.length); i< size; i ++){
...
istream& operator>>(istream& is, const polynomialType& p){
